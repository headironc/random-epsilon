...
const display = computed(() => {
  ...
  return `请${state.currentTake!.qdxh}号${name}至中药房${
    state.currentTake?.window ? state.currentTake?.window + "号窗口" : "3号窗口"
  }取药`;
});
...
function speak(currentTake: Take) {
  state.speech.text = `请${currentTake.qdxh}号${currentTake.brxm}至中药房${
    currentTake?.window ? currentTake?.window + "号窗口" : "3号窗口"
  }取药`;
  state.speech.voice =
    state.voices[
